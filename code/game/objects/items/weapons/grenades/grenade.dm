...
/obj/item/weapon/grenade/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isscrewdriver(W))
		switch(det_time)
			if (1)
				det_time = 10
				user << SPAN_NOTE("You set the [name] for 1 second detonation time.")
			if (10)
				det_time = 30
				user << SPAN_NOTE("You set the [name] for 3 second detonation time.")
			if (30)
				det_time = 50
				user << SPAN_NOTE("You set the [name] for 5 second detonation time.")
			if (50)
				det_time = 1
				user << SPAN_NOTE("You set the [name] for instant detonation.")
		add_fingerprint(user)
	..()
	return
